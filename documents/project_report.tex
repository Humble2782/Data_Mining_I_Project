...
\section{Data Mining}
First, we partitioned the dataset into training and testing subsets, using observations from 2019 to 2022 as the training data and the year 2023 as the test set. During model selection and hyperparameter tuning, only the training data was used, while the test set was kept strictly separate to prevent any data leakage and ensure an unbiased assessment of selected models during the final evaluation. In addition to the simple majority-vote baseline, we implemented a domain-specific baseline that relies exclusively on the speed-limit feature to predict accident severity. This rule-based model assumes that higher speed limits correspond to a greater likelihood of severe accidents. 

Our primary metric for both model selection and evaluation was the macro F1-score. Since the task is an ordinal classification problem, metrics such as weighted Cohen’s kappa can also provide useful insights and were considered during evaluation. However, for our use case, correctly identifying the severely injured class is particularly critical. The consequences of failing to identify a severely injured person are far more severe than those of mistakenly classifying someone as injured when they are not. Therefore we placed special emphasis on the recall of the serverly injured class and relied on macro F1 rather than weighted F1 as  this class is highly underrepresented. 


To gain an initial understanding of the difficulty of the prediction task, we experimented with a broad range of machine learning models while applying only minimal hyperparameter tuning. Specifically, we trained several linear models—including logistic regression, ordinal logistic regression, a lasso classifier and a ridge classifier. In addition, we experimented with decision trees (as the only symbolic approach) and ensemble-based methods such as Random Forest classifiers. We further tested multiple gradient boosting algorithms, including HistGradientBoosting and CatBoost, and also trained a simple neural network.

Across these experiments, we observed that all machine learning models substantially outperformed the baseline methods. However, model performance appeared to reach an upper bound: none of the approaches achieved a macro F1-score above approximately 0.7. Additionally we found that models that explicitly leveraged the ordinal structure of the target variable did not achieve better performance than the other approaches. Based on these observations, we applied Occam’s Razor to narrow our focus to three representative models for more extensive fine-tuning and thorough evaluation.
\begin{itemize}
\item \textbf{Ridge Classifier:} Ridge classification provides fast and computationally efficient training and involves only a single hyperparameter to tune. Because ridge regularization tends to perform well when many features contribute small effects, it aligned well with our assumption that the predictive signal in our dataset arises from a broad set of variables rather than a few dominant predictors.
\item \textbf{Balanced Random Forest:} Although an individual decision tree performed reasonably, our experiments showed that using an ensemble of trees (i.e., a Random Forest) significantly improved predictive performance while still retaining interpretability through feature importance scores and the option to inspect individual trees. The Balanced Random Forest variant was especially useful: while macro F1 remained comparable, performance on the severely injured class improved substantially, which is crucial for our application—even though slight reductions were observed for the non-injured and slightly-injured classes.

\item \textbf{CatBoost:} CatBoost achieved the strongest performance in our preliminary experiments, making it a natural candidate for deeper analysis. Its ability to efficiently handle categorical features, which constitute the majority of our dataset, combined with fast and stable training are further benefits of this approach.
